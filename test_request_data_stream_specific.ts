...
async function testRequestDataStreamSpecific() {
  ...
  return new Promise<void>((resolve) => {
    ...
    let messageTypeCounts: Record<number, number> = {};
      ...
      setTimeout(() => {
        ...
        streams.forEach((stream, index) => {
          setTimeout(() => {
            ...
            console.log(`📤 Sending REQUEST_DATA_STREAM for ${stream.name} (ID ${stream.id}, ${stream.rate}Hz)...`);
      ...
      for (const msg of messages) {
        ...
        if (msg.message_name === 'GPS_RAW_INT') {
          ...
          if (payload.lat && payload.lon) {
            console.log(`   Lat: ${payload.lat/1e7}°, Lon: ${payload.lon/1e7}°, Alt: ${payload.alt/1000}m`);
          ...
          const payload = msg.payload as any;
          if (payload.lat && payload.lon) {
            console.log(`   Global Pos - Lat: ${payload.lat/1e7}°, Lon: ${payload.lon/1e7}°, Alt: ${payload.alt/1000}m`);
          }
