...
<mavlink>
  ...
  <messages>
    <message id="0" name="TEST_MESSAGE">
      ...
      try {
        const result = await execAsync(`npx tsx ${cliPath} generate -i ${testXmlPath} -o ${testOutputDir} -n test`);
...
<mavlink>
  ...
  <messages>
    <message id="1" name="SIMPLE_MESSAGE">
      ...
      try {
        const result = await execAsync(`npx tsx ${cliPath} generate -i ${testXmlPath} -o ${testOutputDir} -n testsingle -f single`);
...
<mavlink>
  <invalid-xml-structure>
      ...
      try {
        await execAsync(`npx tsx ${cliPath} generate -i ${testXmlPath} -o ${testOutputDir} -n testinvalid`);
        throw new Error('Should have thrown an error');
      } catch (error: any) {
        expect(error.code).toBe(1);
...
<mavlink>
  ...
  <messages>
    <message id="1" name="TEST_MSG">
      ...
      try {
        const result = await execAsync(`npx tsx ${cliPath} generate -i ${testXmlPath} -o ${testOutputDir}`);
  ...
  describe('batch command', () => {
    ...
    test('should generate package.json when --package flag is used', async () => {
      try {
        const result = await execAsync(`npx tsx ${cliPath} batch -o ${testOutputDir} -d minimal --package`);
    ...
    test('should handle batch generation errors', async () => {
      try {
        ...
        expect(errorText).toMatch(/Batch generation failed|Command failed|Error|nonexistentdialect/i);
      }
    });
    ...
    test('should parse dialect format option correctly', async () => {
      try {
        const result = await execAsync(`npx tsx ${cliPath} batch -o ${testOutputDir} -d common -f single`);
        expect(result.stdout).toContain('Format: single');
      } catch (error: any) {
    ...
    <message id="1" name="TEST">
      <field type="uint8_t" name="field">Test</field>
        ...
        try {
          ...
          const result = await execAsync(`npx tsx ${cliPath} generate -i ${testXmlPath} -o ${testOutputDir} --help`);
...
<mavlink>
  ...
  <messages>
    <message id="1" name="OPTIONS_TEST">
      ...
      try {
        const result = await execAsync(`npx tsx ${cliPath} generate -i ${testXmlPath} -o ${testOutputDir} -n optionstest -f separate --no-enums --no-type-guards`);
    ...
    test('should parse all batch command options correctly', async () => {
      try {
        const result = await execAsync(`npx tsx ${cliPath} batch -o ${testOutputDir} -d common,minimal -f single --no-enums --no-type-guards --package`);
