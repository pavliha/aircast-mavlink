 ...
 */
export function corruptMessage(message: any, corruptionType: 'missing_field' | 'invalid_type' | 'out_of_range' = 'missing_field'): any {
 ...
 */
export function messagesEqual(msg1: MAVLinkParsedMessage, msg2: MAVLinkParsedMessage, tolerance = 0.0001): boolean {
 ...
 */
export function groupMessagesByType(messages: MAVLinkParsedMessage[]): Record<string, MAVLinkParsedMessage[]> {
 ...
 */
export function calculateFieldStats(messages: MAVLinkParsedMessage[], fieldName: string): {
  min: number;
  max: number;
  avg: number;
  count: number;
  ...
  switch (message.message_name) {
    ...
    case 'GPS_RAW_INT':
      ...
      if (message.payload.satellites_visible < 0 || message.payload.satellites_visible > 255) {
        errors.push(`GPS_RAW_INT.satellites_visible out of range: ${message.payload.satellites_visible}`);
      }
      break;
  }
