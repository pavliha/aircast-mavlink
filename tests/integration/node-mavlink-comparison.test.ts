...
describe('Node-MAVLink Compatibility Tests', () => {
  ...
  describe('Array Field Ordering', () => {
    it('should serialize PROTOCOL_VERSION with correct field order', () => {
      ...
      expect(Array.from(payload.slice(0, 8))).toEqual([0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF, 0x11, 0x22]);

      // library_version_hash at bytes 8-15
      expect(Array.from(payload.slice(8, 16))).toEqual([0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xAA]);
    ...
    it('should parse PROTOCOL_VERSION with correct field order', () => {
      ...
      expect(decoded.payload.spec_version_hash).toEqual([0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08]);
      expect(decoded.payload.library_version_hash).toEqual([0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18]);
