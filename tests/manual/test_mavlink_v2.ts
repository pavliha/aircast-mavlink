  ...
  return new Promise<void>((resolve) => {
    ...
    const socket = createConnection({ host: 'dev.aircast.one', port: 5760 }, () => {
      ...
      setTimeout(() => {
        ...
        const qgcParams = [
          { target_system: 1, target_component: 0, req_stream_id: 2, req_message_rate: 1, start_stop: 1, name: "Raw sensors" },
          { target_system: 1, target_component: 0, req_stream_id: 3, req_message_rate: 1, start_stop: 1, name: "Extended status" },
          { target_system: 1, target_component: 0, req_stream_id: 10, req_message_rate: 1, start_stop: 1, name: "Extra1" },
          { target_system: 1, target_component: 0, req_stream_id: 0, req_message_rate: 1, start_stop: 1, name: "All streams" }
        ];
        ...
        qgcParams.forEach((params, index) => {
          setTimeout(() => {
            ...
            console.log(`📤 Sending REQUEST_DATA_STREAM - ${params.name} (stream ${params.req_stream_id})...`);
            const frame = serializer.serialize(request);
            console.log(`   Frame: ${Array.from(frame).map(b => '0x' + b.toString(16).padStart(2, '0')).join(' ')}`);
            socket.write(frame);
