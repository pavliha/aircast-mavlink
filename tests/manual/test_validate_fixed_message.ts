...
const correctedBytes = [
  0xfe, 0x06, 0x00, 0xff, 0xbe, 0x42, 0x01, 0x00, 0x01, 0x01, 0x00, 0x01, 0xd8, 0xe3,
]
...
async function runTest() {
  ...
  console.log(
    'Frame bytes:',
    correctedBytes.map((b: number) => '0x' + b.toString(16).padStart(2, '0')).join(' ')
  )
  ...
  try {
    ...
    if (result.success) {
      ...
      if (result.error && result.error.includes('wrong checksum')) {
        ...
        console.log(
          'This suggests there might be a tiny difference in CRC calculation or CRC_EXTRA value.'
        )
