  ...
  return new Promise((resolve, reject) => {
    ...
    client.on('connect', () => {
      ...
      try {
        ...
        console.log('\n📤 Sending HEARTBEAT...');
        console.log('Heartbeat bytes:', Array.from(heartbeatBytes).map(b => '0x' + b.toString(16).padStart(2, '0')).join(' '));
        ...
        setTimeout(() => {
          ...
          console.log('Request bytes:', Array.from(requestBytes).map(b => '0x' + b.toString(16).padStart(2, '0')).join(' '));
          console.log(`Generated checksum: 0x${(requestBytes[requestBytes.length-2] | (requestBytes[requestBytes.length-1] << 8)).toString(16).padStart(4, '0')}`);
    ...
    client.on('data', (data) => {
      ...
      if (data.length >= 8) {
        ...
        console.log(`📋 MAVLink frame: magic=0x${magic.toString(16)}, len=${length}, seq=${sequence}, sys=${systemId}, comp=${componentId}, msg=${messageId}`);
...
testSITLConnection().then(result => {
