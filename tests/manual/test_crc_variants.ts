...
;(() => {
  ...
  function crc3_masked(data: Uint8Array, crcExtra: number): number {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      tmp = (tmp ^ (tmp << 4)) & 0xff
      crc =
        (((crc >> 8) & 0xff) ^ ((tmp << 8) & 0xff00) ^ ((tmp << 3) & 0xff8) ^ ((tmp >> 4) & 0xf)) &
        0xffff
    }
  ...
  function crc4_swapped(data: Uint8Array, crcExtra: number): number {
    const crc = crc1_current(data, crcExtra)
    // Swap bytes
  ...
  console.log(
    `Current:  0x${crc1_current(data, crcExtra).toString(16)} (${crc1_current(data, crcExtra)})`
  )
